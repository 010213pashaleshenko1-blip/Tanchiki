...
function Game({ supabase, playerName, roomCode, onLeave }) {
  ...
  useEffect(() => {
    ...
    channel.on('broadcast', { event: 'crate' }, ({ payload }) => { if (payload?.id && !orbs.current.some((o) => o.id === payload.id)) orbs.current.push(payload); });
  ...
  const spawnXpCrate = (broadcast = true) => {
    const crate = { id: `crate-${roomCode}-${makeId()}`, kind: 'drop', x: rand(450, MAP_W - 450), y: rand(450, MAP_H - 450), value: Math.floor(rand(XP_DROP_MIN, XP_DROP_MAX + 1)), born: performance.now() };
    if (!collidesWithBlock(crate.x, crate.y, 38)) {
  ...
  const collectOrbs = () => {
    ...
    for (const orb of orbs.current) {
      const pickupRadius = orb.kind === 'drop' ? radius + 28 : radius;
