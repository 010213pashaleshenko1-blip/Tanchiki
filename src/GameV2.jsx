...
function Game({ supabase, playerName, roomCode, onLeave }) {
  ...
  const damageMe = (amount) => {
    ...
    if (me.current.hp <= 0) {
      ...
      const dropCount = getDeathDropCount(me.current.level);
      dropXpOrbs(deathX, deathY, dropCount);
      loseLevelsOnDeath();
      me.current.deaths += 1;
