...
function Game({ supabase, playerName, roomCode, onLeave }) {
  ...
  const me = useRef({ id: makeId(), name: playerName, level: 1, xp: 0, x: rand(400, MAP_W - 400), y: rand(400, MAP_H - 400), angle: 0, hp: 100, kills: 0, lastFire: 0, color: `hsl(${Math.floor(rand(0, 360))} 80% 58%)` });
  ...
  const [ui, setUi] = useState({ hp: 100, level: 1, xp: 0 });
  const [mobile, setMobile] = useState(isMobileLike());
  const [portrait, setPortrait] = useState(innerHeight > innerWidth && isMobileLike());

  const syncUi = () => setUi({ hp: me.current.hp, level: me.current.level, xp: me.current.xp });
  ...
  const damageMe = (amount) => {
    ...
    syncUi();
    if (me.current.hp <= 0) { me.current.x = rand(400, MAP_W - 400); me.current.y = rand(400, MAP_H - 400); me.current.hp = getMaxHp(me.current); setTimeout(syncUi, 350); }
  };
  ...
  const dropXpOrbs = (x, y) => {
    for (let i = 0; i < ORBS_PER_KILL; i++) {
      const a = rand(0, Math.PI * 2), d = rand(24, 66);
      orbs.current.push({ id: makeId(), x: clamp(x + Math.cos(a) * d, 20, MAP_W - 20), y: clamp(y + Math.sin(a) * d, 20, MAP_H - 20), value: 1, born: performance.now() });
    }
  };

  const update = (dt, now, fire) => {
    updatePlayer(dt, fire); updateBots(dt, now); collectOrbs();
    ...
    for (const b of bullets.current) {
      if (b.owner === me.current.id) {
        ...
        for (const bot of bots.current) if (bot.hp > 0 && Math.hypot(b.x - bot.x, b.y - bot.y) < getRadius(bot) + 8) { bot.hp -= b.damage || getDamage(me.current, 34); b.born = 0; if (bot.hp <= 0) { me.current.kills += 1; dropXpOrbs(bot.x, bot.y); respawnBot(bot); } }
      } else if (String(b.owner).startsWith('bot-') && Math.hypot(b.x - me.current.x, b.y - me.current.y) < getRadius(me.current) + 8) { b.born = 0; damageMe(b.damage || 15); }
    }
  };

  const collectOrbs = () => {
    const radius = getRadius(me.current) + 20;
    const keep = [];
    for (const orb of orbs.current) { if (Math.hypot(orb.x - me.current.x, orb.y - me.current.y) <= radius) addXp(orb.value); else keep.push(orb); }
    orbs.current = keep;
  };
  ...
  const updateBots = (dt, now) => {
    for (const bot of bots.current) {
      ...
      if (dist < 1400 && now - bot.lastFire > BOT_FIRE_COOLDOWN) { bot.lastFire = now; const s = getScale(bot); bullets.current.push({ id: makeId(), owner: bot.id, damage: getDamage(bot, 15), x: bot.x + Math.cos(bot.angle) * 32 * s, y: bot.y + Math.sin(bot.angle) * 32 * s, vx: Math.cos(bot.angle) * 520, vy: Math.sin(bot.angle) * 520, born: now }); }
    }
  };
